...
4. Run `make jobs` in another terminal to start the Solid Queue worker
...
app/
  ...
  models/
    ...
    assessment.rb              status state + enqueues the job on create
  services/
    affordability_calculator.rb  pure PORO; the only place the rules live
  jobs/
...
1. The controller persists a `MortgageApplication`.
2. It calls `create_assessment!`, which inserts a row in `pending`.
3. `Assessment`'s `after_create_commit` enqueues `AssessAffordabilityJob`
   via Solid Queue.
4. The worker runs `AffordabilityCalculator` against the application and
...
- **Pure calculator service.** `AffordabilityCalculator` is a plain Ruby
  object that takes any object responding to the five domain attributes
  and returns an immutable `Data` result. No Rails, no ActiveRecord, no
  IO. This keeps the rules trivially unit-testable and easy to evolve in
  isolation from controllers and persistence.
...
- **Assumption: the rules are illustrative.** LTV ≤ 90%, DTI ≤ 40%,
  ...
  top of `AffordabilityCalculator` and are intended to be replaced.
- **Assumption: no auth.** Every endpoint is open. See "Scaling" below.

## How the assessment logic works

The calculation is in `app/services/affordability_calculator.rb`. Given
...
writes the five result fields and sets `status: :completed`; on any
exception it sets `status: :failed`, stores `error_message`, and
re-raises so Active Job's retry/backoff applies.

## Running the test suite

```bash
make test     # bundle exec rspec inside the web container
make lint     # rubocop
